...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode="random", fused_modconv=True, gain=1):
        ...
        if self.use_noise and noise_mode == "random":
            ...
            # )
            # adapted to allow arbitrary image size at test time
            noise = (
...
class Generator(torch.nn.Module):
    ...
    def mean_ws(self, n_latent):
        latent_in = torch.randn(n_latent, self.z_dim, device=self.mapping.device)
        mean_ws = self.mapping(latent_in).mean(0, keepdim=True)
        return mean_ws
